...
if (!fs.existsSync(postsDir)) {
  console.log(
    "[GeneratePosts] pages 디렉토리가 없습니다. 빈 posts.json을 생성합니다."
  );
...
const posts = files.map((filename) => {
  ...
  console.log(`[GeneratePosts] 처리 중: ${filename}`);

  // Front Matter 파싱 (Windows 줄바꿈 처리)
  ...
  console.log(`[GeneratePosts] ${filename} 처리 완료: ${post.title}`);
...
console.log(`[GeneratePosts] posts.json 생성 완료: ${posts.length}개 게시글`);
